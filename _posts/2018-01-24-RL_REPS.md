...
<iframe width="1120" height="630" src="https://www.youtube.com/embed/OnKi7wqan2A" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
...
* Step 5 **Optimize dual function**
    ...
    * $g(\eta, v) =  \eta \epsilon + \max{\delta_v} + \eta \log \sum_{\boldsymbol{x^{[i]}, u^{[i]}}} \frac{1}{N} Z_i $
    * $\nabla_{\eta} g(\eta, \boldsymbol{v}) = \epsilon + \log \sum_{i} (\frac{1}{N} Z_i) - \frac{\sum_i Z_i (\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max \delta_v)}{\eta \sum_i Z_i} $ 
    * $\nabla_{\boldsymbol{v}} g(\eta, \boldsymbol{v}) = \frac{\sum_i Z_i (\mathbb{E_{p(\boldsymbol{x'\mid x^{[i]},u^{[i]}})}} [\varphi(\boldsymbol{x}')] - \varphi(\boldsymbol{x}^{[i]})) }{\sum_i Z_i} \quad $ with $ \quad Z_i = \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max \delta_v }{\eta})} $
...
### **Appendix 2:** Optimizing the Dual Function [Derivation]
...
&=& \frac{\sum 1/N \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max{\delta_v}} {\eta})} \nabla_{\boldsymbol{v}} \delta_{\boldsymbol{v}} (x^{[i]}, u^{[i]})}{\sum 1/N \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max{\delta_v}} {\eta})}} \\

&=& \frac{\sum_i Z_i (\mathbb{E_{p(\boldsymbol{x'|x^{[i]},u^{[i]}})}} [\varphi(\boldsymbol{x}')] - \varphi(\boldsymbol{x}^{[i]})) }{\sum_i Z_i} \quad \text{with} \quad Z_i = \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max \delta_v }{\eta})}
\end{eqnarray}
$$



### Appendix : Gaussian policy 
