...
* Step 6 **Policy Update** by Weighted Maximum Likelihood estimate
    ...
    * $$
      \begin{eqnarray}
      \boldsymbol{\Sigma_{\text{new}}} = \frac{\sum_i^N d^{[i]} (\boldsymbol{a}^{[i]} - \boldsymbol{a}^T\phi(s^{[i]})) (\boldsymbol{a}^{[i]} - \boldsymbol{W}^T\phi(s^{[i]}))^T}{Z}
      \end{eqnarray}
      $$
    * $$
      \begin{eqnarray}
...
0 &=& \sum_i^{N} d^{[i]} [\boldsymbol{\Sigma^{-1} a^{[i]} \phi(s^{[i]}) - \Sigma^{-1} W^T\phi(s^{[i]})\phi(s^{[i]})^T} ]\\
 ...
 \boldsymbol{A D \Phi^T} &=& \boldsymbol{ W^T\Phi D \Phi^T} \\
 \boldsymbol{W^T} &=& \boldsymbol{ A D \Phi^T (\Phi D \Phi^T)^{-1}}  \\
 \boldsymbol{W} &=& \boldsymbol{ (A D \Phi^T (\Phi D \Phi^T)^{-1})^T}  \\
 \boldsymbol{W} &=& \boldsymbol{ ((\Phi D \Phi^T)^{-1})^T (A D \Phi^T)^T}  \\
 \boldsymbol{W} &=& \boldsymbol{ ((\Phi D \Phi^T)^{T})^{-1} (\Phi D^T A^T)}  \\
 \boldsymbol{W} &=& \boldsymbol{ (\Phi^T D \Phi)^{-1} (\Phi D A^T)}  \\
\end{eqnarray}
$$
