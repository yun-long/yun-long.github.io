...
* Step 5 **Optimize dual function**
    ...
    * $g(\eta, v) = \eta \epsilon + \eta \log (\frac{1}{N} \sum_{i=1}^{N}\exp(\frac{\delta_{v}^{[i]}}{\eta})) =  \eta \epsilon + \max{\delta_v} + \eta \log \sum_{\boldsymbol{x^{[i]}, u^{[i]}}} \frac{1}{N} \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max{\delta_v}} {\eta})} $
    ...
    * $\nabla_{\boldsymbol{v}} g(\eta, \boldsymbol{v}) = \eta \frac{\sum_i Z_i (\mathbb{E_{p(\boldsymbol{x'\mid x^{[i]},u^{[i]}})}} [\varphi(\boldsymbol{x}')] - \varphi(\boldsymbol{x}^{[i]})) }{\sum_i Z_i} $ with $Z_i = \exp{(\frac{\delta_{\boldsymbol{v}} (\boldsymbol{x^{[i]}, u^{[i]}}) - \max \delta_v }{\eta})} $
