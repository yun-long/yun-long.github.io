...
We formulate this as conditional distribution modeling. From a dataset $\mathcal D=\{(o_i,a_i)\}$, learn $p_\theta(a\mid o)$ to approximate the demonstrated action distribution $p_{\mathrm{data}}(a\mid o)$. Flow matching is the method we will use to train and sample this policy.

## 1. What are we trying to model?

At robot timestep $t$, let $o_t$ contain the available observations, such as camera images, proprioception, or a history of these measurements. One training target is an *action chunk* of horizon $H$:

$$
a_t=(u_t,u_{t+1},\ldots,u_{t+H-1})\in\mathbb R^{H\times d_u},\qquad
\mathcal D=\{(o_i,a_i)\}_{i=1}^N.
$$

Here $u_t\in\mathbb R^{d_u}$ is one action; $a$ denotes the whole chunk. We can flatten it into $\mathbb R^d$, with $d=Hd_u$, for the derivation. The pairs describe a joint data distribution $p_{\mathrm{data}}(o,a)$. At deployment, the observation is given, so our target is its conditional distribution:
...
**Why a distribution?** A deterministic predictor trained with squared error has the population optimum $f^*(o)=\mathbb E[A\mid o]$. When the conditional action distribution has separated modes, this mean can lie in a low-density region. A generative policy instead aims to sample coherent chunks from the conditional distribution. Flow matching gives us a tractable way to train such a sampler.
...
A neural network $v_\theta(x,s;o)\in\mathbb R^d$ predicts a velocity in *action-chunk space*. Its inputs are the current noisy chunk $x$, flow time $s$, and observation $o$. Generate the chunk with an ODE:

$$
\frac{dZ_s}{ds}=v_\theta(Z_s,s;o),\qquad Z_0\sim p_0,\qquad \hat a=Z_1.
$$

**Two different times:** $t$ indexes robot execution; $s\in[0,1]$ indexes the internal generation process. Every flow step refines the *entire* $H$-action chunk. The observation stays fixed during this solve. The predicted flow velocity is not itself a physical robot velocity command.

Write $q_s^\theta(\cdot\mid o)=\operatorname{Law}(Z_s\mid o)$. We want $q_1^\theta(\cdot\mid o)\approx p_{\mathrm{data}}(\cdot\mid o)$. We now have a conditional sampler, but no target for its velocity.
...
$$
X_s=(1-s)X_0+sA,\qquad p_s(\cdot\mid o)=\operatorname{Law}(X_s\mid o).
$$
...
[Interactive figure: Conditional action density and training versus ODE paths, holding the observation fixed](../figures/flow-matching.html?view=transport)
  ...
  Hold $o$ fixed and move flow time from 0 to 1. The upper curve is the exact toy $p_s(x\mid o)$. Below it, compare training interpolation with the ODE of the exact marginal velocity. These are paths through action space during generation, not robot trajectories through physical space. No neural network is trained in this figure.

$p_s(\cdot\mid o)$ is our chosen target; $q_s^\theta(\cdot\mid o)$ is what the model produces. Also, the law of interpolated samples is generally different from the density mixture $(1-s)p_0+s\,p_{\mathrm{data}}(\cdot\mid o)$.
...
$$
\begin{aligned}
 p_s(x\mid a,o)=p_s(x\mid a)&=\mathcal N\!\left(x;sa,(1-s)^2I_d\right),\\
 u_s(x\mid a)&=\frac{a-x}{1-s},\qquad 0\le s<1.
\end{aligned}
$$

Obtain the second line by solving $x=(1-s)x_0+sa$ for $x_0$, then substituting into $a-x_0$. On a sampled training path, the target is simply $a-X_0$. This endpoint difference avoids dividing by $1-s$.

At inference, $o$ is available but the desired endpoint $a$ is unknown. We need a velocity depending on $(x,s,o)$, without access to that endpoint.
...
$$
\begin{aligned}
 p_s(x\mid o)&=\int p_s(x\mid a)p_{\mathrm{data}}(a\mid o)\,da,\\
 u_s(x;o)&=\int u_s(x\mid a)\,
 \underbrace{\frac{p_s(x\mid a)p_{\mathrm{data}}(a\mid o)}{p_s(x\mid o)}}_{p(a\mid X_s=x,o)}\,da\\
 &=\mathbb E[A-X_0\mid X_s=x,o].
\end{aligned}
$$
...
\begin{aligned}
  \partial_s p_s(x\mid o)
  &=\int \partial_s p_s(x\mid a)p_{\mathrm{data}}(a\mid o)\,da\\
  &=-\nabla_x\cdot\int p_s(x\mid a)u_s(x\mid a)p_{\mathrm{data}}(a\mid o)\,da\\
  &=-\nabla_x\cdot\big(p_s(x\mid o)u_s(x;o)\big).
  \end{aligned}
$$

  An ODE using $u_s(\cdot;o)$ starts from the same $p_0$ and has the same density evolution when the equation has a unique solution. It preserves the conditional time marginals, not the training endpoint pairings.
...
The ideal loss regresses onto $u_s(x;o)$, but its posterior integral is intractable. Use the sampled action endpoint directly. Draw $(o,A)$ from the joint data distribution, $X_0\sim\mathcal N(0,I_d)$ independently, and $s\sim\mathcal U(0,1)$:

$$
\begin{aligned}
 X_s&=(1-s)X_0+sA,\\
 \mathcal L_{\mathrm{CFM}}(\theta)
 &=\mathbb E_{(o,A),X_0,s}\left[
 \left\|v_\theta(X_s,s;o)-(A-X_0)\right\|^2\right].
\end{aligned}
$$
...
To see why the loss works, let $Y=A-X_0$, $v=v_\theta(X_s,s;o)$, and $u=\mathbb E[Y\mid X_s,s,o]$. Then
...
The cross term vanishes because $\mathbb E[Y-u\mid X_s,s,o]=0$. The two losses have the same expected gradient. At the unrestricted population optimum, the network predicts the desired observation-conditioned marginal velocity. Finite data, capacity, and optimization introduce approximation error.
...
- **Train:** draw a pair $(o,a)$, independent noise $x_0$, and flow time $s$. Form $x_s=(1-s)x_0+sa$. Predict $v_\theta(x_s,s;o)$, regress against $a-x_0$, and update $\theta$.

- **Predict a chunk:** obtain the current observation $o_t$, draw fresh $z_0\sim\mathcal N(0,I_d)$, and integrate while keeping $o_t$ fixed. With $K$ Euler steps:

$$
z_{k+1}=z_k+\frac1K\,v_\theta\!\left(z_k,\frac{k}{K};o_t\right),\quad k=0,\ldots,K-1,\qquad \hat a_t=z_K.
$$

Reshape $\hat a_t$ into $H$ actions. A controller can execute a prefix of length $H_{\mathrm{exec}}\le H$, collect a new observation, and generate another chunk. The execution horizon $H_{\mathrm{exec}}$, prediction horizon $H$, and number of flow steps $K$ are separate choices.
